    ...
    if system[0].dimension == 3:
        ...
        for k in range(array_size):
            for l in range(array_size): 
                ...
                curvature1[:,k,l] = np.sqrt(d2f_dBx2_1**2 + d2f_dBy2_1**2 + d2f_dBz2_1**2)
                curvature2[:,k,l] = np.sqrt(d2f_dBx2_2**2 + d2f_dBy2_2**2 + d2f_dBz2_2**2)
                curvature3[:,k,l] = np.sqrt(d2f_dBx2_3**2 + d2f_dBy2_3**2 + d2f_dBz2_3**2)

        return Trans_Eng_1,gradient1,curvature1,Trans_Eng_2,gradient2,curvature2,Trans_Eng_3,gradient3,curvature3
    
    elif system[0].dimension == 2:

        # Initialize arrays with proper dimensions
        array_size = int(Energies.shape[2]/2)
        Trans_Eng = np.zeros((neighbour_arr.shape[2], array_size, array_size), dtype=np.float32)
        gradient = np.zeros((neighbour_arr.shape[2], array_size, array_size), dtype=np.float32)
        curvature = np.zeros((neighbour_arr.shape[2], array_size, array_size), dtype=np.float32)
        # Initialize arrays with proper dimensions
        array_size = Energies.shape[2] // 2  # Ensuring integer division
        m = Energies.shape[-1] // 2  # Integer division for valid indexing
        
        for k in range(array_size):
            for l in range(array_size): 
                # First derivatives (central differences)
                #for ms=-1/2 to ms=1/2 transition
                df_dBx_1 = ((Energies[:,1,m+l] - Energies[:,2,m+l]) - (Energies[:,1,k] - Energies[:,2,k])) / (2*delta_Bx)
                df_dBy_1 = ((Energies[:,3,m+l] - Energies[:,4,m+l]) - (Energies[:,3,k] - Energies[:,4,k])) / (2*delta_By)           
                df_dBz_1 = ((Energies[:,5,m+l] - Energies[:,6,m+l]) - (Energies[:,5,k] - Energies[:,6,k])) / (2*delta_Bz)
                ...
                # Curvature calculation
                # numerator   = (df_dBy_1**2 + df_dBz_1**2) * d2f_dBx2_1 + (df_dBx_1**2 + df_dBz_1**2) * d2f_dBy2_1 + (df_dBx_1**2 + df_dBy_1**2) * d2f_dBz2_1 + 2 * df_dBx_1 * df_dBy_1 * d2f_dBxdBy_1 + 2 * df_dBx_1 * df_dBz_1 * d2f_dBxdBz_1 + 2 * df_dBy_1 * df_dBz_1 * d2f_dBydBz_1
                # denominator = 2 * (df_dBx_1**2 + df_dBy_1**2 + df_dBz_1**2)**(1.5)
                 
                # Store results
                Trans_Eng[:,k,l] = Energies[:,0,m+l] - Energies[:,0,k]
                gradient[:,k,l] = (np.sqrt(df_dBx_1**2 + df_dBy_1**2 + df_dBz_1**2))
                # curvature[:,k,l] = np.abs(numerator / denominator)
                curvature[:,k,l] = np.sqrt(d2f_dBx2_1**2 + d2f_dBy2_1**2 + d2f_dBz2_1**2)

        return Trans_Eng,gradient,curvature
